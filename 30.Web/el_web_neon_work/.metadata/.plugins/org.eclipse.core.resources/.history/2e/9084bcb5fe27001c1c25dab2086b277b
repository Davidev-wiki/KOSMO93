...
<%
	...
	// 현대 페이지에서 사용할 바인딩 객체 생성
	// fruit가 다른 페이지에서 넘겨받았으면 바인딩 객체를 생성할 필요가 없으나
	// 현재 페이지에서 만들었으므로 EL 표기법에서 사용하기 위해 생성할 필요가 있다.
%>
<!DOCTYPE html>
<html>
<head>
<meta charset="EUC-KR">
<title>jstl_06 반복문</title>
</head>
<body>
<h3>jstl_06</h3>
<ul>
...
<%--
	JSTL반복문 : <c:forEach>실행문</c:forEach>
	<c:forEach var="반복변수명" items="${배열명}">실행문</c:forEach>
 --%>
JSTL 반복문(배열)<br/>
<ul>
	<c:forEach var="i" items="${fruit}"><li>${i }</li>
	</c:forEach>
</ul>
</body>
</html>